...
Plane Runway::return_plane(int runway)
{
	Plane rolling_plane;
	if ( runway = 1 )
	{
		landing.serve_and_retrieve(rolling_plane);
		return rolling_plane;
	}
	// else if (runway = 2)
	else
	{
		takeoff.serve_and_retrieve(rolling_plane);
		return rolling_plane;
	}
}
void Runway::append_plane(int runway, Plane rolling_plane )
{
	if (runway = 1)
	{
		landing.append(rolling_plane);
	}
	// else if (runway = 2)
	else
	{
		takeoff.append(rolling_plane);
	}
}